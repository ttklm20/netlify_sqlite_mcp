...
def create_fund_database():
    ...
    CREATE TABLE IF NOT EXISTS funds (
        ...
        week_growth_rate REAL,
        month_growth_rate REAL,
        three_month_growth_rate REAL,
        six_month_growth_rate REAL,
        one_year_growth_rate REAL,
        two_year_growth_rate REAL,
        three_year_growth_rate REAL,
        ytd_growth_rate REAL,
        since_inception_growth_rate REAL,
        issue_date TEXT,
        purchasable INTEGER,
        custom_field2 REAL,
        custom_field3 REAL,
...
def insert_fund_data(fund_data):
    ...
    try:
        ...
        INSERT OR REPLACE INTO funds (
            fund_code, fund_name, fund_abbr, update_date, unit_net_value,
            cumulative_net_value, daily_growth_rate, week_growth_rate,
            month_growth_rate, three_month_growth_rate, six_month_growth_rate,
            one_year_growth_rate, two_year_growth_rate, three_year_growth_rate,
            ytd_growth_rate, since_inception_growth_rate, issue_date,
            purchasable, custom_field2, custom_field3, fee_rate, discount,
        ...
        ''', (
            ...
            fund_data.get('基金简拼'),
            fund_data.get('更新日期'),
            clean_numeric_value(fund_data.get('单位净值').replace('%', '') if fund_data.get('单位净值') else None),
            clean_numeric_value(fund_data.get('累计净值').replace('%', '') if fund_data.get('累计净值') else None),
            ...
            clean_int_value(fund_data.get('折扣', 0)),
            clean_percentage_value(fund_data.get('自定义5')),
            clean_int_value(fund_data.get('自定义6', 0))
        ))
...
def query_fund_data(fund_code=None, fund_name_keyword=None):
    ...
    if fund_code:
        ...
        # 按基金名称模糊查询
        cursor.execute('SELECT * FROM funds WHERE fund_name LIKE ? ORDER BY fund_code', 
                      (f'%{fund_name_keyword}%',))
    else:
        # 查询所有数据
...
def fuzzy_search_funds(keyword):
    ...
    # 使用LIKE进行模糊匹配
    cursor.execute('''
        SELECT fund_code, fund_name, fund_abbr, unit_net_value, daily_growth_rate
        FROM funds 
        WHERE fund_name LIKE ? OR fund_abbr LIKE ? OR fund_code LIKE ?
        ORDER BY fund_code
...
def display_fund_data(column_names, data):
    ...
    for row in data:
        for i, value in enumerate(row):
            # 格式化显示百分比数据
            if column_names[i] in ['daily_growth_rate', 'week_growth_rate', 'month_growth_rate', 
                                  'three_month_growth_rate', 'six_month_growth_rate', 'one_year_growth_rate',
                                  'two_year_growth_rate', 'three_year_growth_rate', 'ytd_growth_rate',
                                  'since_inception_growth_rate', 'custom_field3', 'fee_rate', 'custom_field5']:
